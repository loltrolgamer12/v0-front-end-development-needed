...
con normalización mejorada de placas.
"""
import pandas as pd
import numpy as np
...
def load_inspection_data(file_path: str) -> pd.DataFrame:
    ...
    df['PLACA DEL VEHICULO'] = df['PLACA DEL VEHICULO'].fillna('').apply(normalize_plate)
    
    # Guardar placas originales y normalizadas para diagnóstico
    placas_diferentes = df[df['PLACA DEL VEHICULO'] != df['PLACA DEL VEHICULO'].fillna('')]['PLACA DEL VEHICULO'].unique()
    if len(placas_diferentes) > 0:
        print("\nPlacas normalizadas:")
        for placa in placas_diferentes:
    ...
    return df[mascara_placas_validas]
...
def main():
    ...
    for year in sorted(df['fecha'].dt.year.unique()):
        ...
        for month in sorted(df[df['fecha'].dt.year == year]['fecha'].dt.month.unique()):
